...
def get_table_client():
    ...
    """
    connection_string = os.environ["ORDERS_STORAGE_CONNECTION"]
...
# ============================================================
# 5. ANALISAR PEDIDO COM AZURE OPENAI
# ============================================================

@activities.activity_trigger(input_name="order")
def analyze_order_with_ai(order: dict):
    order_id = str(order.get("order_id"))

    log_event(
        "info",
        "Iniciando análise do pedido com Azure OpenAI",
        event="ai_analysis_started",
        activity="analyze_order_with_ai",
        order_id=order_id,
        status="STARTED"
    )
    ...
    try:
        prompt = f"""
Analise o pedido abaixo e retorne uma classificação simples.

Pedido:
- ID: {order.get("order_id")}
- Cliente: {order.get("customer")}
- Produto: {order.get("product")}
- Quantidade: {order.get("quantity")}

Determine:
1. Uma categoria para o pedido.
2. Um nível de prioridade: LOW, MEDIUM ou HIGH.
...
"""
        ...
        log_event(
            "error",
            "Falha na análise com Azure OpenAI",
...
def finish_order(order):
    ai_result = order.get("ai_result", {})
    order_data = order.get("order", order)

    order_id = str(order_data.get("order_id"))

    # --------------------------------------------------------
    # EVENTO: FINALIZACAO INICIADA
    # --------------------------------------------------------
